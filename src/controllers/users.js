...
function SignUp(req, res) {
  ...
  bcrypt.hash(User.password, bcrypt.genSaltSync(7), (err, hash) => {
    ...
    User.save((err) => {
      if (err) {
        return res.status(204).send({message: 'Error creating account'});
      } else {
  ...
  UsersSchema.findOne({alias: alias}, (err, User) => {
    if (!User) {
      ...
      bcrypt.compare(password, User.password, function(err, match) {
        if (!match) {
          return res.status(204).send({message: 'Passwords do not match'});
        } else {
...
function ListUsers(req, res) {
  UsersSchema.find({status: true}, (err, Users) => {
    if (Users.length == 0) {
      return res.status(204).send({message: 'No users to show'});
    } else {
